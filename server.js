...
//   const {item, name,price  } = request.body
...
//   const item = parseInt(request.params.id)
//   const { name,price } = request.body

//   pool.query(
//     'UPDATE hotel SET name = $1 WHERE item = $2',
//     [name,price, item],
//     (error, results) => {
//       if (error) {
//         throw error
    ...
    switch(request.method){
        case "GET":
            if(request.url=="/employees"){
                ...
                response.write(JSON.stringify({code:100,name:"Akshata",city:"Hubli",phone:"9988998899"}));
                response.end();
            }
            case "POST":
            case "DELETE":
